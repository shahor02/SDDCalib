...
//--------------------------------------------------------------------
TString defStorage="";
//TString defOCDB = "local:///cvmfs/alice-ocdb.cern.ch/calibration/data/2018/OCDB";
TString defOCDB = "raw://";
...
double minBEntryX = 5;        // min entries in the bin to account (vs Xdrift)
double minBEntryZ = 10;        // min entries in the bin to account (vs Z)
...
Int_t  userRebinX = 1;
Int_t  userRebinZ = 2;
Float_t userMinEntriesPerBin = 5; // assume histo empty if there are < 20 entries/bin in average
  ...
  for (int imd=kSDDMin;imd<=kSDDMax;imd++) {
    for (int ix=0;ix<2;ix++) {
      ...
      PrepareModuleHistos(imd,ix, kTRUE);
        if (!ProcSDDSideDXvsX()) {printf("!ProcSDDSideDXvsX()");return;}     // process DX vs X histos to get corr.maps, deltaT0, deltaX0, deltaV_mean
      StoreCurrent();
    }
...
AliCDBEntry* GetCDBEntry(const char* path, const char* objType)
{
  ...
  if (strp=="auto" && objType) {
    ...
    man->SetDefaultStorage(defStorage.IsNull() ? defOCDB.Data() : defStorage.Data());
    ...
    if (!stor && !man->GetRaw()) man->SetDefaultStorage(defOCDB.Data());
