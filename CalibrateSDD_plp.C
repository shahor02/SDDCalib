...
*/
//
const double kMaxChi2SimpleMap = 0.5; // if polinomial fit gives chi2 below this value, make map out of it
...
int    nVDEdge    = 7;        // min width in n bins for VDrift averaging region on the edges
...
Int_t  userRebinX = 1;
Int_t  userRebinZ = 2;
Float_t userMinEntriesPerBin = 15; // assume histo empty if there are < 20 entries/bin in average
Float_t userMaxEmptyFrac = 0.8;    // assume histo empty if there are > than fraction low-stat bins
...
AliITSresponseSDD* sddResp=0;             // SDD response, old, then updated
AliITSresponseSDD *respForceT0 = 0;       // SDD response for forced t0
...
TH1*       resT0CorrImp = 0;                 // correction to modules T0 actually imposed (different from resT0Corr if forceT0=kTRUE)
...
//
Double_t   imposePileUpShiftT0 =  0;     // impose pile-up effect of t0 shift (in ns)
Double_t   meanVDforPileUpShift = 6.;     // mean vdrift for accounting pile-up effect
//
TString pathSDDRespOld    = "";
TString pathSDDVDriftOld  = "";
TString pathSDDCorrMapOld = "";
TString pathSDDForceT0 = "";
TString pathSDDForceMap = "";

TObjArray* oldMap = 0;
TObjArray* forcedMap = 0;
//
TString pathDeDxFile = "";
//
//--------------------------------------------------------------------
void      CalibrateSDD(const char* qaOutFile,int rMin=-1,int rMax=-1, int useRunForAuto=-1,
		       const char* pathSDDResp=0, const char* pathSDDVDrift=0, const char* pathSDDCorrMap=0, 
		       const char* corrdEdx="CorrectiondEdxSDD_LHC11h_pass1mrgTot_Run166530_999999999_v4_s0.root",
		       const char* forceT0from=0,
		       const char* forceMap=0,
		       //const char* collName="clistITSAlignQA"   
		       const char* collName="clistSDDCalib"  //for pass0
		       );
//
void      Process(TCollection* qa, int runMin=0,int runMax=-1,int useRunForAuto=-1,const char* pathSDDResp=0,const char* pathSDDVDrift=0, const char* pathSDDCorrMap=0,
		  const char* corrdEdx=0,const char* forceT0from=0,const char* forceMap=0);
void      Process();
...
void      PlotReport(const char* psname=0);
...
Bool_t    CureEdges(TH1* profH1, TProfile* profP);
...
Bool_t    HistoFromMap(TH1* mapHisto, AliITSCorrMap1DSDD* mp);
//
...
AliCDBEntry* GetCDBEntry(const char* path, const char* objType=0);
//
...
void                UpdateSDDResponse(Bool_t t0=kTRUE, Bool_t vdrift=kTRUE);
AliITSCorrMap1DSDD* CreateCorrMap(TH1* mapHisto, int imd, int side);
void      PrepCDBObj(TObject *obj,const char* path,int firstrun=0,int lastrun=999999999,const char* comment="");
void      StoredEdxParams(AliITSresponseSDD *resp, TString &corrFileName);
void      PileUpEffectX(TH1* resid, double t0shift, double v);


//-------------------------------------------------------------------
void CalibrateSDD(const char* qaOutFile,int rMin,int rMax, int useRunForAuto,
		  const char* pathSDDResp, const char* pathSDDVDrift,const char* pathSDDCorrMap, 
		  const char* corrdEdx, const char* forceT0from,
		  const char* forceMap,
		  const char* collName
		  )
{
  ...
  if (!fl) {printf("Failed to open qa output %s\n",fs.Data()); exit(1);}
  if (fl->Get("ITSAlignQA")) fl->cd("ITSAlignQA");
  TCollection *qa = (TCollection*)gDirectory->Get(collName);
  if (!qa) {printf("Did not fine %s in qa output %s\n",collName,fs.Data());}
  Process(qa,rMin,rMax,useRunForAuto,pathSDDResp, pathSDDVDrift,pathSDDCorrMap, corrdEdx,forceT0from,forceMap);
}

//_________________________________________________________________________
void Process(TCollection* qa, int rMin,int rMax,int useRunForAuto,const char* pathSDDResp, const char* pathSDDVDrift, 
	     const char* pathSDDCorrMap, const char* corrdEdx, const char* forceT0from,const char* forceMap)
{
  // process all
  qaHistos = qa;
  ...
  pathSDDForceT0     = forceT0from;
  pathSDDForceMap    = forceMap;
  //
  firstRun = rMin;
  lastRun  = rMax;
  runForAuto = useRunForAuto;
  if (runForAuto<0) runForAuto = firstRun;
  //
  printf("Will prepare objects for run range %d %d, runAuto=%d\n",firstRun,lastRun,runForAuto);
  pathDeDxFile = corrdEdx;
  Process();
}
...
void Process()
{
  ...
  if ( (pathSDDRespOld.BeginsWith("alien:") || 
	pathSDDVDriftOld.BeginsWith("alien:") || 
	pathSDDCorrMapOld.BeginsWith("alien:") ||
	pathSDDForceT0.BeginsWith("alien:") ||
	pathSDDForceMap.BeginsWith("alien:")	
	)
       && !gGrid) {
    TGrid::Connect("alien://");
    if (!gGrid) {printf("No Grid connection\n"); exit(1);}
  }
  //
  if (!pathSDDForceT0.IsNull()) {
    if (!LoadSDDResponse(pathSDDForceT0, respForceT0)) exit(1);    
  }
  //
  if (!pathSDDRespOld.IsNull()) {
    if (!LoadSDDResponse(pathSDDRespOld, sddResp)) exit(1);    
  }
  
  //
  if (!pathSDDCorrMapOld.IsNull()) {
    if (!LoadSDDCorrMap(pathSDDCorrMapOld,oldMap)) exit(1);
  }
  //
  if (!pathSDDForceMap.IsNull()) {
    if (!LoadSDDCorrMap(pathSDDForceMap,forcedMap)) exit(1);
    forceT0CorrTo0 = kFALSE;
    forceRPhiCorrTo0 = kFALSE;
  }
  //
  ...
  for (int imd=kSDDMin;imd<=kSDDMax;imd++) {
    for (int ix=0;ix<2;ix++) {
      ...
      if (!ProcSDDSideDXvsX()) return;     // process DX vs X histos to get corr.maps, deltaT0, deltaX0, deltaV_mean
      StoreCurrent();
    }
...
Bool_t ProcSDDSideDXvsX()
{
  ...
  PileUpEffectX(currHDXvsXCorr,imposePileUpShiftT0,meanVDforPileUpShift);
  //
  if ( (currHDXvsXclean=CleanProfile(currHDXvsX)) ) {
    //
    delete currHDXvsXCorr;
    PileUpEffectX(currHDXvsXclean,imposePileUpShiftT0,meanVDforPileUpShift);
    //
    if (oldMap) {
      currHCorrMapX = (TH1*)currHDXvsXclean->Clone("oldCorr");
      HistoFromMap(currHCorrMapX,(AliITSCorrMap1DSDD*)oldMap->At(2*(currSDDId-kSDDMin) + currSDDSide));
      printf("Undoing old map effect\n");
      currHDXvsXclean->Add(currHCorrMapX);
      delete currHCorrMapX; currHCorrMapX = 0;
    }
    ...
    // try simple solution
    if (forcedMap) {
      TString nm = currHDXvsXclean->GetName(); nm += "_map";
      currHCorrMapX = (TH1*)currHDXvsXclean->Clone(nm.Data());
      HistoFromMap(currHCorrMapX,(AliITSCorrMap1DSDD*)forcedMap->At(2*(currSDDId-kSDDMin) + currSDDSide));
      printf("Will use forced map\n");
    }
    else if (!(currHCorrMapX=SimpleMap(currHDXvsXclean))) { 
  ...
  // check results
  if (!CheckResultDX()) return kFALSE;
  //
  return kTRUE;
}
...
Bool_t ProcSDDSideDXvsZ()
{
  ...
  //
  double plpShiftX = imposePileUpShiftT0*meanVDforPileUpShift;
  //
  for (int ib=ib0;ib<=ib1;ib++) {
    double ne = currHTDvsZ->GetBinEntries(ib);
    if (ne<1) continue;
    double dx = currHDXvsZ->GetBinContent(ib) + plpShiftX;   // raw residual X vs Z
    double dxe= currHDXvsZ->GetBinError(ib);     
    double t  = currHTDvsZ->GetBinContent(ib) + imposePileUpShiftT0;   // mean assumed (TDrift - T0)
    double dt = 0;
    if (respForceT0 && sddResp) dt = sddResp->GetTimeZero(currSDDId) - respForceT0->GetTimeZero(currSDDId);
    else                        dt = resT0Corr->GetBinContent(currMod+1);
    t += dt;
...
{
  // clean profile copy
  TH1* prof = ProfileAsTH1(profR,"_h1");
  CureEdges(prof,profR);

  //  TH1* prof = CureFitDXEdges(profR); // cure edges
...
TH1* Resid2Vdrift(TH1* res)
{
  ...
  for (ib=1;ib<nb;ib++) { // detect up to which bin the left tail is unstable
    ...
    double xE = vd->GetBinError(ib);
    double vl = vd->GetBinContent(ib);
    double difV = TMath::Abs(vl-vAv);
    if (difV<0.2) break;
    if (xE>1e-6 && difV/xE<3) { 
  ...
  for (int i=ib;i<=nb && nbcnt<nbfl;i++) {
    ...
    double difV = TMath::Abs(vd->GetBinContent(i)-vAv);
    if (berr<1e-9 || berr>0.5 || 
	(difV>0.2 && difV/berr<3 )) continue;
  ...
  for (int i=1;i<=ib;i++) vd->SetBinContent(i, vAv);//vL);
  // for safety check if there are no outliers in first 3 "stable" bins
  for (int i=ib+1;i<ib+3;i++) {
    double berr = vd->GetBinError(i);
    double difV = TMath::Abs(vd->GetBinContent(i)-vL);
    if (  berr<1e-9 || (difV>0.2 && difV/berr<3) ) vd->SetBinContent(i, vL);
  }
  ...
  for (ib=nb+1;ib--;) { // detect up to which bin the right tail is unstable
    ...
    double difV = TMath::Abs(vd->GetBinContent(ib)-vAv);
    if (difV<0.4) break;
    if (xE>1e-6 && difV/xE<3) { 
  ...
  for (int i=ib;i>=1 && nbcnt<nbfl;i--) {
    double berr = vd->GetBinError(i);
    double difV = TMath::Abs(vd->GetBinContent(i)-vAv);
    if (berr<1e-9 || berr>0.5 || (difV>0.4 && difV/berr<3  )) continue;
    vR += vd->GetBinContent(i)/berr/berr;
    eR += 1./berr/berr;
    nbcnt++;
    lastCounted = i;
    //    printf("%d %e %e %e\n",i, vd->GetBinContent(i),difV,vR/eR);
  }
  vR = eR>0 ? vR/eR : vAv;
  ...
  for (int i=(lastCounted+ib)/2;i<ib;i++) {
    double berr = vd->GetBinError(i);
    double difV = TMath::Abs(vd->GetBinContent(i)-vR);
    if (  berr<1e-9 || (difV>0.2 && difV/berr<3)) vd->SetBinContent(i, vR);
  }
...
Bool_t CheckResultDX()
{
  ...
  /*
  if (currSDDId==346) {
    currHDXvsX->Fit(fitP0,"","");
  double offsRaw1 = fitP0->GetParameter(0);
  currHDXvsXCorr->Fit(fitP0,"","same");
  double offsAMap1 = fitP0->GetParameter(0);
  printf("Mod %d %d %d, press C to continue\n",currSDDId, currSDDSide,currMod );
  if (getchar()!='c') return kFALSE;
  }
  */
  ...
  int ip = 0;
  double meanDxx=0,meanTDx=0,meanTXCorr=0;
  for (int i=b0;i<b1;i++) {
    if (currHTDvsX->GetBinEntries(i)<minBEntryX) continue;
    double t = currHTDvsX->GetBinContent(i) + imposePileUpShiftT0;
    double x = currHDXvsXCorr->GetBinCenter(i); // here the pile-up is already accounted
    if (tmin>t) tmin = t;
    if (tmax<t) tmax = t;
    if (xmin>x) xmin = x;
    if (xmax<x) xmax = x;    
    currGrDxx->SetPoint(ip,x,currHDXvsXCorr->GetBinContent(i));
    double err = currHDXvsX->GetBinError(i)>1e-6 ? currHDXvsX->GetBinError(i) : 1;
    currGrDxx->SetPointError(ip,currHDXvsXCorr->GetBinWidth(i)/2,err);
    //
    currGrTDx->SetPoint(ip,t, currHDXvsXCorr->GetBinContent(i));
    currGrTDx->SetPointError(ip, currHTDvsX->GetBinError(i), err);    
    //
    ...
    //
    meanDxx += currHDXvsXCorr->GetBinContent(i);
    meanTDx += currHDXvsXCorr->GetBinContent(i);
    meanTXCorr += currHTDvsX->GetBinCenter(i);
    ip++;
  }
  if (ip>0) {
    meanDxx/=ip; 
...
void CalcDXCorrections()
{
  ...
  //
  if (!resT0CorrImp) {
    resT0CorrImp = new TH1F("T0CorrImp","T0 Correction Imposed",kNSDD,kSDDMin-0.5,kSDDMax+0.5);
    resT0CorrImp->SetMarkerColor(kRed);
    resT0CorrImp->SetMarkerStyle(24);    
  }
  //
  if (!resXLocCorr) {
...
TObjArray* CreateCorrMaps()
{
  ...
  printf("Creating correction maps\n");
  ...
  for (int imd=kSDDMin;imd<=kSDDMax;imd++) {
    for (int side=0;side<2;side++) {
      ...
      AliITSCorrMap1DSDD* mp = CreateCorrMap(mph,imd,side);
...
AliITSCorrMap1DSDD* CreateCorrMap(TH1* mapHisto, int imd, int side)
{
  // create correction map from histo
  //printf("Beginning of createcorrmaps \n");  
  ...
  //printf("after inversion bit \n");  
  if (mapHisto) for (int ib=b0;ib<=b1;ib++) {
      //  printf("%d %d %d\n",ib,b0,b1);printf(">>%d %e\n",ib,mapHisto->GetBinContent(ib)); 
      mpCorr->SetCellContent(0,ib-b0,-mapHisto->GetBinContent(ib));
    }

  //
  //printf("End of createcorrmaps \n");  
  return mpCorr;
}
...
Bool_t HistoFromMap(TH1* mapHisto, AliITSCorrMap1DSDD* mp)
{
  ...
  b0 = mapHisto->FindBin(1);
  b1 = mapHisto->FindBin(sddSeg.Dx()-1);
  int nbOld = mp->GetNBinsDrift();
  double dx = sddSeg.Dx()/nbOld;
  mapHisto->Reset();
  for (int ib=b0;ib<=b1;ib++) {
    double x = mapHisto->GetBinCenter(ib);
    int ibm = x/dx;
    double mpVal = mp->GetCellContent(0,ibm);
    mapHisto->SetBinContent(ib, -mpVal);
    mapHisto->SetBinError(ib, 0);    
  }
  return kTRUE;
}
...
//_________________________________________________________________________________
void UpdateSDDResponse(Bool_t t0, Bool_t vdrift)
{
  // retrieve RespSDD object and update it
  UpdateSDDResponse(sddResp, t0, vdrift);
  sddRespUpdT0OK = t0;
  sddRespUpdVDOK = vdrift;
  //
  StoredEdxParams(sddResp, pathDeDxFile);
  //
}
...
void UpdateSDDResponse(AliITSresponseSDD *resp, Bool_t t0, Bool_t vdrift)
{
  ...
  for (int imd=kSDDMin;imd<=kSDDMax;imd++) {
    if (t0) {
      double t0CorrImp = 0;
      if (respForceT0) t0CorrImp = resp->GetTimeZero(imd) - respForceT0->GetTimeZero(imd);
      else             t0CorrImp = resT0Corr->GetBinContent(imd-kSDDMin+1);
      //
      resT0CorrImp->SetBinContent(imd-kSDDMin+1,t0CorrImp);
      resp->SetModuleTimeZero(imd, resp->GetTimeZero(imd) - t0CorrImp);
    }
    if (vdrift) {
      for (int ix=0;ix<2;ix++) {
...
Bool_t LoadSDDVDrift(TString& path, TObjArray *&arr)
{
  ...
  while(1) {
    //
    if (path=="auto") {
      entry = GetCDBEntry(path.Data(),"ITS/Calib/DriftSpeedSDD");  
...
Bool_t LoadSDDResponse(TString& path, AliITSresponseSDD *&resp)
{
  ...
  while(1) {
    //
    if (path=="auto") {
      entry = GetCDBEntry(path.Data(),"ITS/Calib/RespSDD");
...
Bool_t LoadSDDCorrMap(TString& path, TObjArray *&maps)
{
  ...
  while(1) {
    //
    if (path=="auto") {
      entry = GetCDBEntry(path.Data(),"ITS/Calib/MapsTimeSDD");     
...
AliCDBEntry* GetCDBEntry(const char* path, const char* objType)
{
  ...
  //
  TString strp = path;
  if (strp.IsNull()) return 0;
  if (strp=="auto" && objType) {
    if (man->GetEntryCache()->Contains(objType)) man->UnloadFromCache(objType);
    TString defStorURI = man->GetDefaultStorage() ? man->GetDefaultStorage()->GetURI() : "";
    man->UnsetDefaultStorage();
    man->SetDefaultStorage(defStorage.IsNull() ? "raw://" : defStorage.Data());
    man->SetRun(runForAuto);
    printf("Use run %d (auto mode) for CDB acces to %s from %s\n",runForAuto,objType, man->GetDefaultStorage()->GetURI().Data());
    entry = man->Get(objType);
    return entry;
    man->ClearCache();
    man->UnsetDefaultStorage();
    if (!defStorURI.IsNull()) man->SetDefaultStorage(defStorURI.Data());
    //
  }
  else {
...
void PlotReport(const char* psname)
{
  ...
  if (psnm1.IsNull()) psnm1 = Form("sddQAreport_%d_%d.ps",firstRun,lastRun);
  TString psnm0 = psnm1 + "["; 
  TString psnm2 = psnm1 + "]";
  ...
  PlotHisto(resT0CorrImp,"p same",24,kRed,0.5);
  AddPadLabel("T0 Correction Found", 0.2,0.93,kBlue,0.05);
  AddPadLabel("Imposed", 0.7,0.93,kRed,0.05);
  if (TMath::Abs(imposePileUpShiftT0)>0.01) AddPadLabel(Form("Imposed Plp #DeltaT0=%.1f <V>=%.1f",imposePileUpShiftT0,meanVDforPileUpShift), 
							0.2,0.88,kBlue,0.05);
  ...
  for (int imd=kSDDMin;imd<=kSDDMax;imd++) {
	...
	hsdd->GetXaxis()->SetRange(ib0,ib1);
      ...
      //
      AddPadLabel(Form("#DeltaV:%+.4f | #DeltaT0:%+5.0f (Imp:%+5.0f) | #DeltaX:%+4.0f",
		       resVDCorr[ix] ? resVDCorr[ix]->GetBinContent(imd-kSDDMin+1):0,
		       resT0Corr     ?     resT0Corr->GetBinContent(imd-kSDDMin+1):0,
		       resT0CorrImp  ?  resT0CorrImp->GetBinContent(imd-kSDDMin+1):0,
...
void PrepCDBObj(TObject *obj,const char* path,int firstrun,int lastrun,const char* comment)
{
  ...
  printf("Writing object %s, runs: %d:%d |Comment: %s\n",path,firstrun,lastrun,comment);
  //
  AliCDBManager* man = AliCDBManager::Instance();
  man->UnsetDefaultStorage();
...
Bool_t CureEdges(TH1* profH1, TProfile* profP)
{
  ...
  //
  const double kLftEdge = 1800.;
  const double kRgtEdge = 1800.;
  const double kBinThresh = 0.2; // bin is ok if has at least this fraction of av. counts/bin
  int nb = profH1->GetNbinsX();
  int b0 = profH1->FindBin(0.+1e-6);
  int b1 = profH1->FindBin(sddSeg.Dx() - 1e-6);
  //
  // find leftmost significant bin (at least 20% of expected bin counts)
  double avStat = 0;
  int nBadBins = 0;
  for (int i=b0;i<=b1;i++) {
    int nent = profP->GetBinEntries(i);
    avStat += nent;
    if (nent<userMinEntriesPerBin) nBadBins++;
  }
  float empFrac = float(nBadBins)/(b1-b0+1);
  printf("Fraction of bad bins %f for %s <%f>\n",empFrac,profH1->GetName(),avStat/(b1-b0+1));
  if (empFrac>userMaxEmptyFrac) {
    printf("Fraction of bad bins %f for %s, assume histo empty\n",empFrac,profH1->GetName());
    profH1->Reset();
    return kFALSE;
  }
  avStat /= (b1-b0+1);
  double threshL = kBinThresh*avStat;
  double threshR = kBinThresh*avStat;
  int b0v,b1v;
  for (b0v=b0;b0v<b1;b0v++) if (profP->GetBinEntries(b0v)>threshL) break;
  for (b1v=b1;b1v>b0;b1v--) if (profP->GetBinEntries(b1v)>threshR) break;
  if (b0v>=b1v || (b0v>int(nb*0.9)) || (b1v<int(nb*0.1)) ) {
    printf("Anomaly: Left edge: %d (%f) Right edge %d (%f)\n", b0v,profH1->GetBinCenter(b0v),b1v,profH1->GetBinCenter(b1v));
    return kFALSE;
  }
  // substitute the left  edge points by minimum within kLftEdge dist
  // for anomalous modules (large lacoons) take the calue of 1st bin with av.stat
  double minV = 1e9, maxV = -1e9; 
  int minB = 9999, maxB=-9999;
  if (b0v<int(0.3*nb)) {
    int b0e = profH1->FindBin( profH1->GetBinCenter(b0v)+kLftEdge );    
    for (int i=b0v;i<=b0e;i++) {
      if ( (profP->GetBinEntries(i)>threshL) && (profP->GetBinContent(i)<minV) ) {
	minV = profP->GetBinContent(i);
	minB = i;  
      }
    }
  }
  else {
    for (minB=b0v;minB<b1v;minB++) if (profP->GetBinEntries(minB)>avStat) {minV = profP->GetBinContent(minB); break;}
  }
  for (int i=1;i<=minB;i++) profH1->SetBinContent(i,minV);
  //
  // substitute the right edge points by maximum within kRgtEdge dist
  // for anomalous modules (large lacoons) take the calue of 1st bin with av.stat
  if (b1v>int(0.7*nb)) {
    int b1e = profH1->FindBin( profH1->GetBinCenter(b1v)-kRgtEdge );
    for (int i=b1v;i>=b1e;i--) {
      if ( (profP->GetBinEntries(i)>threshR) && (profP->GetBinContent(i)>maxV) ) {
	maxV = profP->GetBinContent(i);
	maxB = i;  
      }
    }
  }
  else {
    for (maxB=b1v;maxB>b0v;maxB--) if (profP->GetBinEntries(maxB)>avStat) {maxV = profP->GetBinContent(maxB); break;}
  }
  for (int i=nb;i>=maxB;i--) profH1->SetBinContent(i,maxV);
  //
  return kTRUE;
}
...
void SafeRebin(TProfile* prof, Int_t factor, Bool_t xprof)
{
  ...
  //
  if ( verbose && ((xprof&&firstX) || ((!xprof)&&firstZ)) ) printf("Rebin%c: Will rebin %d to %d\n",xprof ? 'X':'Z',nbUse,nbUseNew);
  if (factor<2) return;
  //
...
TH1* SimpleMap(TH1* prof)
{
  ...
  //
  double lmax = sddSeg.Dx();
  if (lft>=0.3*lmax || rgt<0.7*lmax) return 0; // don't use simple map for incomplete sensors
...
void StoredEdxParams(AliITSresponseSDD *resp, TString &corrFileName)
{
  // Attach the dEdX params to response object
  printf("Adding dEdX params from %s\n",corrFileName.Data());
  if (corrFileName.IsNull()) {
    printf("No dEdX correction file supplied\n");
    return;
  }
  gSystem->ExpandPathName(corrFileName);
  //
  TFile* fil = TFile::Open(corrFileName.Data());
  if (!fil) {
    printf("Failed to open dEdX correction file %s\n",corrFileName.Data());
    exit(1);
  }
  TH1F* hadc=(TH1F*)fil->Get("hADCtokeV");
  TH1F* hsl=(TH1F*)fil->Get("hADCvsDriftTime");

  for(Int_t iMod=240; iMod<500; iMod++){
    if(hadc){
      Int_t theBin=hadc->FindBin(iMod);
      Float_t adc=hadc->GetBinContent(theBin);
      //      printf("%d  %f\n",iMod,adc);
      resp->SetADCtokeV(iMod,adc);
    }
    if(hsl){
      Int_t theBin=hsl->FindBin(iMod);     
      Float_t sl=hsl->GetBinContent(theBin);
      resp->SetADCvsDriftTime(iMod,sl);
    }
  }
  //
  printf("Added dEdX info from %s\n",corrFileName.Data());
}
...
void PileUpEffectX(TH1* resid, double t0shift, double v)
{
  // account for t0 shift due to the pile-up
  int nb = resid->GetNbinsX();
  double shiftX = t0shift*v;
  for (int i=1;i<=nb;i++) {
    double res = resid->GetBinContent(i);
    resid->SetBinContent(i, res + shiftX);
  }
}
